...
function PolkaApp(){
  ...
  const [openGroupDetailId,setOpenGroupDetailId]=useState(null);
  ...
  useEffect(()=>{
    ...
    Promise.all([getSession(),loadFeaturePreferences(),loadAppFeatureFlags(),Linking.getInitialURL()]).then(async([session,prefs,flags,url])=>{
      if(!alive)return;
  ...
  useEffect(()=>{
    if(tab!=='Plany')return;
    setMountedPlanViews(prev=>{
      if(prev.has(plansView))return prev;
      const next=new Set(prev);next.add(plansView);return next;
    });
  },[tab,plansView]);
  ...
  if(!loaded||booting)return (
    <View style={s.splashRoot}>
      <StatusBar barStyle="light-content" backgroundColor="#C84F7A"/>
  ...
  if(!account){
    ...
    if(!entryStarted)return <View style={s.safe}><StatusBar barStyle="light-content" translucent backgroundColor="transparent"/><WelcomeScreen authFlags={appFeatureFlags} onContinue={({method}={})=>{if(method==='skip')setEntryStarted(true)}} onBusiness={()=>setPreBusiness(true)}/></View>;
  ...
  const overlayContent=reportTarget?
  ...
  return <SafeAreaView edges={showTabs?['top']:['top','bottom']} style={s.safe}><StatusBar barStyle="dark-content" backgroundColor={c.canvas}/>
    <ShiftTransition screenKey={screenKey}>
      <View style={s.fill}>
        ...
        {mountedTabs.has('Start')&&<View style={[s.fill,{display:showTabs&&tab==='Start'?'flex':'none'}]}><CommunityScreen city={activeCity} sessionUserId={authSession?.user?.id||null} posts={posts} setPosts={setPosts} blockedIds={blockedIds} onReport={setReportTarget} onOpenChat={openDirectChat}/></View>}
        {mountedTabs.has('Poznaj')&&<View style={[s.fill,{display:showTabs&&tab==='Poznaj'?'flex':'none'}]}><PeopleDiscoverScreen city={activeCity} sessionUserId={authSession?.user?.id||null} onMessage={openDirectChat} onOpenProfile={profile=>setPublicProfileTarget({id:profile.id,name:profile.name,photo:profile.photo})} blockedIds={blockedIds} zodiacEnabled={featurePreferences.zodiacPeopleMatching} userZodiac={account?.zodiac||featurePreferences.zodiacSign} styleEnabled={featurePreferences.stylePeopleMatching} userStyle={account?.style||featurePreferences.stylePreference} onBlock={block} onReport={setReportTarget}/></View>}
        ...
        {mountedTabs.has('Grupy')&&<View style={[s.fill,{display:showTabs&&tab==='Grupy'?'flex':'none'}]}><ClubsMeetupsScreen key={session} city={activeCity} sessionUserId={authSession?.user?.id||null} onReport={setReportTarget} onOpenChat={openGroupChat} openGroupId={openGroupDetailId} onGroupOpened={()=>setOpenGroupDetailId(null)}/></View>}
...
const s=StyleSheet.create({safe:{flex:1,backgroundColor:c.canvas},fill:{flex:1},brandTopBar:{height:68,paddingHorizontal:sp.lg,flexDirection:'row',alignItems:'center',justifyContent:'space-between',backgroundColor:c.canvas},brandLogo:{fontFamily:f.bold,fontWeight:"800",fontSize:31,letterSpacing:-1.5,color:c.ink},cityBlock:{flexDirection:'row',alignItems:'center',gap:6},cityName:{fontFamily:f.bold,fontSize:27,letterSpacing:-1.1,color:c.ink},cityModalBackdrop:{flex:1,backgroundColor:'rgba(0,0,0,.28)',justifyContent:'flex-start',alignItems:'flex-end',paddingTop:82,paddingRight:16},citySheet:{width:230,backgroundColor:c.white,borderRadius:22,padding:14,shadowColor:'#000',shadowOpacity:.14,shadowRadius:24,shadowOffset:{width:0,height:10},elevation:8},citySheetTitle:{fontFamily:f.bold,fontSize:17,color:c.ink,paddingHorizontal:6,paddingBottom:8},cityOption:{height:46,flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:8,borderRadius:12},cityOptionText:{fontFamily:f.semibold,fontSize:15,color:c.ink},floatingChatFab:{position:'absolute',right:18,zIndex:99,width:56,height:56,borderRadius:28,backgroundColor:c.pink,alignItems:'center',justifyContent:'center',shadowColor:'#C84F7A',shadowOpacity:0.38,shadowRadius:14,shadowOffset:{width:0,height:6},elevation:9},chatDot:{position:'absolute',top:11,right:12,width:9,height:9,borderRadius:4.5,backgroundColor:c.white,borderWidth:2,borderColor:c.pink},tabBar:{backgroundColor:c.white,borderTopWidth:1,borderColor:c.line,flexDirection:'row',paddingTop:sp.md,paddingHorizontal:sp.xs},tab:{flex:1,alignItems:'center',justifyContent:'center',gap:4,minHeight:48},tabText:{fontSize:10,color:c.muted,fontFamily:f.semibold},plansSwitch:{marginHorizontal:sp.lg,marginBottom:10,padding:4,borderRadius:16,backgroundColor:c.white,borderWidth:1,borderColor:c.line,flexDirection:'row'},plansSwitchItem:{flex:1,height:38,borderRadius:12,alignItems:'center',justifyContent:'center'},plansSwitchActive:{backgroundColor:c.blush},plansSwitchText:{fontFamily:f.semibold,fontSize:13,color:c.muted},plansSwitchTextActive:{fontFamily:f.bold,color:c.pink},
  ...
  splashRoot:{flex:1,backgroundColor:'#C84F7A',alignItems:'center',justifyContent:'center'},
