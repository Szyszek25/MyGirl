...
function PolkaApp(){
  ...
  return <SafeAreaView edges={showTabs?['top']:['top','bottom']} style={s.safe}><StatusBar barStyle="dark-content" backgroundColor={c.canvas}/>
    <ShiftTransition screenKey={screenKey}>
      <View style={s.fill}>
        {showTabs&&<View style={s.brandTopBar}><Typography style={s.brandLogo}>Polka</Typography><View style={s.cityBlock}><Ionicons name="location-outline" size={18} color={c.pink}/><Typography style={s.cityName}>{account?.city||'Warszawa'}</Typography></View></View>}
