...
const [account,setAccount]=useState(null);const [tab,setTab]=useState('Odkrywaj');
if(!loaded)return <View style={{flex:1,backgroundColor:c.canvas}}/>;
if(!account)return <SafeAreaView style={s.safe}><StatusBar barStyle="dark-content" backgroundColor={c.canvas}/><Onboarding onComplete={setAccount}/></SafeAreaView>;
const screens={'Odkrywaj':<DiscoverScreen/>,'Social':<CommunityScreen/>,'Grupy':<GroupsScreen/>,'Czaty':<ChatsScreen/>,'Profil':<ProfileScreen account={account} onReset={()=>{setAccount(null);setTab('Odkrywaj')}}/>};
return <SafeAreaView style={s.safe}><StatusBar barStyle="dark-content" backgroundColor={c.canvas}/><View style={{flex:1}}>{screens[tab]}</View><View style={s.tabBar}>{tabs.map(item=><Pressable key={item.key} accessibilityRole="tab" accessibilityLabel={item.key} accessibilityState={{selected:tab===item.key}} onPress={()=>setTab(item.key)} style={s.tab}><Ionicons name={tab===item.key?item.active:item.icon} size={23} color={tab===item.key?c.pink:c.muted}/><Typography style={[s.tabText,tab===item.key&&{color:c.pink,fontFamily:f.bold}]}>{item.key}</Typography></Pressable>)}</View></SafeAreaView>
}
