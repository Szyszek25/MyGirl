...
export default function NativeProfile({account,showCare=true,onSafety,onPartner,onSettings,onCycle,onCare,onMore,onSave,onSignOut,businessAccount,onProfileModeChange}){
  const [editing,setEditing]=useState(false);
  const [shareOpen,setShareOpen]=useState(false);
  const [draft,setDraft]=useState(()=>copy(account));
  const [busy,setBusy]=useState(false);
  const [notice,setNotice]=useState('');
  const [profileMode,setProfileMode]=useState('private');
  ...
  const galleryPhotos=(account?.galleryPhotos||[]).map(item=>item?.url||item).filter(Boolean);
  const profilePhotos=galleryPhotos.length?galleryPhotos:[account?.photo].filter(Boolean);
  const save=async()=>{
    if(busy)return;
  ...
  return <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':undefined}>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={s.page}>
      <View style={s.profileTop}>{businessAccount&&<View style={s.profileModeSwitch}><Pressable onPress={()=>{setProfileMode('private');onProfileModeChange?.('private')}} style={[s.profileModeOption,profileMode==='private'&&s.profileModeActive]}><Ionicons name="person-outline" size={16} color={profileMode==='private'?c.white:c.ink}/><Typography style={[s.profileModeText,profileMode==='private'&&s.profileModeTextActive]}>Prywatne</Typography></Pressable><Pressable onPress={()=>{setProfileMode('business');onProfileModeChange?.('business')}} style={[s.profileModeOption,profileMode==='business'&&s.profileModeActive]}><Ionicons name="storefront-outline" size={16} color={profileMode==='business'?c.white:c.ink}/><Typography style={[s.profileModeText,profileMode==='business'&&s.profileModeTextActive]}>{businessAccount.name}</Typography></Pressable></View>}</View>
      ...
      <>
        ...
        <View style={s.infoTiles}>
        ...
        </View>{!!profilePhotos.length&&<Surface>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.profileGalleryRail} contentContainerStyle={s.profileGalleryRailContent}>{profilePhotos.map((uri,index)=><Image key={uri||index} source={{uri}} style={s.profileGalleryPhoto} resizeMode="cover"/>)}</ScrollView>
        </Surface>}
        ...
        {showCare&&<ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.quickRail} contentContainerStyle={s.quickRailContent}>
          <Pressable onPress={onCycle} style={s.quickCard}><View style={s.quickIcon}><Ionicons name="calendar-outline" size={22} color={c.pink}/></View><View><Typography style={s.quickTitle}>Kalendarz cyklu</Typography><Typography style={s.quickSub}>Cykl i objawy</Typography></View><Ionicons name="chevron-forward" size={18} color={c.muted}/></Pressable>
          <Pressable onPress={onCare} style={s.quickCard}><View style={s.quickIcon}><Ionicons name="heart-circle-outline" size={22} color={c.pink}/></View><View><Typography style={s.quickTitle}>Polka Care</Typography><Typography style={s.quickSub}>Wiedza i wellbeing</Typography></View><Ionicons name="chevron-forward" size={18} color={c.muted}/></Pressable>
        </ScrollView>}
