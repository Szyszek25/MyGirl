...
export function SafetyCenter({blockedIds,onUnblock,reports,onClose,onReset,online=false}){
  ...
  return <ScrollView contentContainerStyle={s.page}>
    ...
    <Surface><Typography variant="subtitle">{online?'Moje zgłoszenia':'Zgłoszenia lokalne'}</Typography><Typography style={s.note}>{reports.length===0?'Brak zgłoszeń.':`${reports.length} zgłoszeń.`}</Typography></Surface>
    <Surface><Typography variant="subtitle">Twój profil i dane</Typography><Typography style={s.note}>{online?'Zarządzanie kontem i usunięcie konta znajdziesz w Ustawieniach.':'Dane profilu są zapisane na tym urządzeniu.'}</Typography>{!online&&<Button title="Usuń profil i dane z urządzenia" secondary icon="trash-outline" onPress={confirmReset}/>}</Surface>
  </ScrollView>;
}
