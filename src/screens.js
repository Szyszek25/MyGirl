...
export function ChatsScreen({ conversationBackRef = null, sessionUserId = null, initialConversationId = null, initialMeetupId = null, blockedIds = [], supportChat = true, onReport, onClose, onConversationStateChange, onOpenChat, onOpenMeetup, onOpenProfile, onOpenGroup }) {
  ...
  useEffect(() => {
    if (!conversationBackRef) return undefined;
    conversationBackRef.current = () => {
      setActive(null);
      setChatProfileOpen(false);
      setPreviewImage(null);
    };
    return () => { conversationBackRef.current = null; };
  }, [conversationBackRef]);
...
const s = StyleSheet.create({
  ...
  composerTrigger: { height: 54, backgroundColor: c.white, borderWidth: 1, borderColor: c.line, borderRadius: 18, flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 12, paddingVertical: 0 },
  composerAvatar: { width: 34, height: 34, borderRadius: 17, backgroundColor: c.blush, alignItems: 'center', justifyContent: 'center' },
  composerPlaceholder: { flex: 1, fontFamily: f.regular, fontSize: 14, lineHeight: 20, color: c.muted, includeFontPadding: false, textAlignVertical: 'center' },
